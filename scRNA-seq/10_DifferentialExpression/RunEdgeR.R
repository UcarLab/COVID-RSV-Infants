...
getFilteredSamples = function(cursamples, cellgroup, mincells=25, filter=TRUE){
  rv = c()
  group_fraction = 0
...
filterGenes = function(curdata, genepercentages, samples, fraccells=0.20, minsamplesingroup=3){
  if(length(samples) < minsamplesingroup){
    return(c())
  }
  ...
  rownames(genepercentages)[rowSums(genepercentages[, headers] > fraccells) >= minsamplesingroup]
}
...
runEdgeR = function(curdata, cellgroup, header, c1, c2, c1_label, c2_label, conditionmap, minsamplesingroup=3){
  ...
  genepercentages = read.csv(paste0("/Users/athib/Desktop/CovidRSV/scRNA2/DE/PercentExpressed/",cellgroup,"_percents.csv"), row.names=1)
...
ncomparisons=32
conditionmap = read.csv("/Users/athib/Desktop/CovidRSV/scRNA2/DE/SampleGroup.csv")
cellnumbers = read.csv("/Users/athib/Desktop/CovidRSV/scRNA2/DE/ClusterStats.csv", row.names = 1)


for(cursample in rownames(cellnumbers)){
#for (cursample in c('CD16_Monocyte_Merged_ISG')){
  edgermatrix = paste0("/Users/athib/Desktop/CovidRSV/scRNA2/DE/EdgeRMatrixFiles/", cursample, ".txt_edgeRMatrix.txt")
  dir.create(file.path("/Users/athib/Desktop/CovidRSV/scRNA2/DE/EdgeROutputs/", cursample))
  setwd(paste0("/Users/athib/Desktop/CovidRSV/scRNA2/DE/EdgeROutputs/", cursample, "/"))
  
  
  unformattedheader = read.csv(edgermatrix, header = FALSE, nrows = 1)
  unformattedheader = unformattedheader[!is.na(unformattedheader)]
  cellgroup = strsplit(basename(edgermatrix), '\\.')[[1]][1]
  curdata = read.csv(edgermatrix, row.names=1)
  ...
  samples_rsvg1g2 = conditionmap$Sample[conditionmap$Group %in% c("RSV G1", "RSV G2")]
  ...
  rv = runEdgeR(curdata, cellgroup, unformattedheader, samples_rsvg1g2, samples_rg3, "RSVG1G2", 'RSVG3', conditionmap)
  summary_comparison[comparisonindex] = rv[1]
  summary_np1[comparisonindex] = rv[2]
  summary_np05[comparisonindex] = rv[3]
  summary_c1samples[comparisonindex] = rv[4]
